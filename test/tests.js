...
describe("Chain JS", function () {
	...
	it("should have properties", function () {
		...
		properties.forEach(function (property) {
			(chain).should.have.property(property);
		});
	});

	describe("crypto.js", function () {
		var crypto = chain.crypto;
	...
	describe("dapp.js", function () {
		...
		describe("#createDApp", function () {
			...
			var options = {
				"name": "chain-dapp-cctime",
				"link": "https://github.com/chainPlatform/chain-dapp-cctime/archive/master.zip",
				"category": 1,
				"description": "Decentralized news channel",
				"tags": "chain,dapp,demo,cctime",
			...
			describe("returned dapp", function () {
				var keys = chain.crypto.getKeys("secret");
				var secondKeys = chain.crypto.getKeys("secret 2");
				...
				it("should be signed correctly", function () {
					var result = chain.crypto.verify(trs);
					(result).should.be.ok;
				});
				...
				it("should be second signed correctly", function () {
					trs.amount = 0;
					var result = chain.crypto.verifySecondSignature(trs, secondKeys.publicKey);
					(result).should.be.ok;
				});
				...
				it("should be ok to verify bytes", function () {
					...
					var secret = 'secret1'
					var keys = chain.crypto.getKeys(secret)
					var signature = chain.crypto.signBytes(data1, keys)
					var result = chain.crypto.verifyBytes(data1, signature, keys.publicKey)
					result.should.be.ok

					var data2 = new Buffer('a1b2c3d4', 'hex')
					signature = chain.crypto.signBytes(data2, keys)
					result = chain.crypto.verifyBytes(data2, signature, keys.publicKey)
					result.should.be.ok
				})
			});
		});
	...
	describe("delegate.js", function () {
		...
		describe("#createDelegate", function () {
			...
			describe("returned delegate", function () {
				...
				it("should be signed correctly", function () {
					var result = chain.crypto.verify(trs, keys.publicKey);
					(result).should.be.ok;
				});
				...
				it("should not be second signed correctly now", function () {
					trs.amount = 100;
					var result = chain.crypto.verify(trs, secondKeys.publicKey);
					(result).should.be.not.ok;
				});
	...
	describe("transaction.js", function () {
		...
		describe("#createTransaction with second secret", function () {
			...
			var keys = chain.crypto.getKeys(secondSecret);
			...
			describe("returned transaction", function () {
				...
				it("should be second signed correctly", function () {
					var result = chain.crypto.verifySecondSignature(trs, keys.publicKey);
					(result).should.be.ok;
				});
	...
	describe("vote.js", function () {
		...
		describe("#createVote", function () {
			var createVote = vote.createVote,
				vt = null,
				publicKey = chain.crypto.getKeys("secret").publicKey,
			...
			describe("returned vote", function () {
				...
				it("should be signed correctly", function () {
					var result = chain.crypto.verify(vt);
					(result).should.be.ok;
				});

				it("should be second signed correctly", function () {
					var result = chain.crypto.verifySecondSignature(vt, chain.crypto.getKeys("second secret").publicKey);
					(result).should.be.ok;
				});
	...
	describe('crypto sha256 and address', function () {
		it('should be equal to the expected address', function () {
			chain.crypto.getAddress('7a91b9bfc0ea185bf3ade9d264da273f7fe19bf71008210b1d7239c82dd3ad20').should.be.equal('AFbYJhiJb3DXzHy5ZP24mKw21M2dCBJCXP')
			var publicKeyBuffer = new Buffer('7a91b9bfc0ea185bf3ade9d264da273f7fe19bf71008210b1d7239c82dd3ad20', 'hex')
			chain.crypto.getAddress(publicKeyBuffer).should.be.equal('AFbYJhiJb3DXzHy5ZP24mKw21M2dCBJCXP')
		})
	})
